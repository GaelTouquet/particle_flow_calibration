...
plt.title(r"histogram of $E_{true}$ for $(E_{ecal}="+str(np.around(e1,2))+" \rm{(GeV),E_{hcal}="+str(np.around(h1,2))+" \rm{(GeV))$",fontsize=18)
plt.subplot(1,2,2)
...
plt.xlabel(r"$E_{\rm true} \rm{(GeV)$",fontsize=18)
plt.title(r"histogram of $E_{true}$ for $(E_{ecal}="+str(np.around(e2,2))+" \rm{(GeV),E_{hcal}="+str(np.around(h2,2))+" \rm{(GeV))$",fontsize=18)
plt.tight_layout()
plt.show()
