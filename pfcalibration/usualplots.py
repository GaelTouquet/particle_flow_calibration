...
def plot_ecalib_over_etrue_functionof_etrue(calib,dataToPredict):
    ...
    plt.plot(energy,mean_gaussianfit,lw=3,label="mean (gaussian fit)")
    plt.legend(loc='upper right')
    plt.plot([0,200],[1,1],'--',lw = 2,color='yellow')
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    plt.ylabel(r"$E_{\rm calib}/E_{\rm true}$",fontsize=20)
    plt.title(r"$E_{\rm ecal} = 0$",fontsize=20)
    plt.subplot(1,2,2)
    plt.plot(t2,r2,'.',markersize=1)
    plt.axis([0,200,0.5,1.5])
    plt.plot(energy2,mean_gaussianfit2,lw=3,label="mean (gaussian fit)")
    plt.legend(loc='upper right')
    plt.plot([0,200],[1,1],'--',lw = 2,color='yellow')
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    plt.ylabel(r"$E_{\rm calib}/E_{\rm true}$",fontsize=20)
...
def hist_ecalib(calib,dataToPredict):
    ...
    plt.xlabel(r"$E_{\rm calib} \rm{(GeV)}$",fontsize=20)
    plt.title(r"$E_{\rm ecal} = 0$",fontsize=20)
    plt.subplot(2,2,2)
...
def plot_gaussianfitcurve_ecalib_over_etrue_functionof_ecal_hcal(calib,dataToPredict):
    ...
    plt.plot(energy,mean_gaussianfit,lw=3, label = r"$E_{\rm ecal} = 0$")
    plt.plot(energy2,mean_gaussianfit2,lw=3, label = r"$E_{\rm ecal} \neq 0$")
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    plt.title(r"$E_{\rm calib}/E_{\rm true}$",fontsize=20)
    plt.ylabel(r"$<E_{\rm calib}/E_{\rm true}>$",fontsize=20)
    plt.legend(loc='upper right')
    ...
    plt.plot(energy,sigma_gaussianfit,lw=3, label = r"$E_{\rm ecal} = 0$")
    plt.plot(energy2,sigma_gaussianfit2,lw=3, label = r"$E_{\rm ecal} \neq 0$")
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    plt.ylabel(r"$\sigma (E_{\rm calib}/E_{\rm true})$",fontsize=20)
    plt.legend(loc='upper right')
    major_ticks = np.arange(0, 200, 50)
    minor_ticks = np.arange(0, 200, 10)
    ax.set_xticks(major_ticks)
    ...
    plt.plot(energy,reducedChi2,lw=3, label = r"$E_{\rm ecal} = 0$")
    plt.plot(energy2,reducedChi22,lw=3, label = r"$E_{\rm ecal} \neq 0$")
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    ...
    plt.hist(x,bins,label = r"$E_{\rm ecal} = 0$")
    ...
    x = x[np.invert(np.isnan(x))]
    bins = binwidth_array(x,binwidth = 0.5)
    plt.hist(x,bins,label = r"$E_{\rm ecal} \neq 0$")
    plt.xlabel(r"$\chi^2/df$",fontsize=20)
    plt.legend(loc='upper right')
    plt.tight_layout()
...
def plot3D_training(data1):
    ...
    ax.set_title("Training points",fontsize=20)
    ax.set_xlabel(r'$E_{\rm ecal} \rm{(GeV)}$',fontsize=20)
    ax.set_ylabel(r'$E_{\rm hcal} \rm{(GeV)}$',fontsize=20)
    ax.set_zlabel(r'$E_{\rm true} \rm{(GeV)}$',fontsize=20)
    plt.tight_layout()
...
def plot3D_surf(calib):
    ...
    ax.set_title("Calibration surface",fontsize=20)
    ax.set_xlabel(r'$E_{\rm ecal} \rm{(GeV)}$',fontsize=20)
    ax.set_ylabel(r'$E_{\rm hcal} \rm{(GeV)}$',fontsize=20)
    ax.set_zlabel(r'$E_{\rm true} \rm{(GeV)}$',fontsize=20)
    plt.tight_layout()
...
def comparison(calibs,dataToPredict):
    ...
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    plt.ylabel(r"$\sigma(E_{\rm calib}/E_{\rm true})$",fontsize=20)
    plt.title(r"$\sigma(E_{\rm calib}/E_{\rm true})$ for $E_{\rm ecal} = 0$",fontsize=20)
    ...
    plt.xlabel(r"$E_{\rm true} \rm{(GeV)}$",fontsize=20)
    plt.ylabel(r"$\sigma(E_{\rm calib}/E_{\rm true})$",fontsize=20)
    plt.title(r"$\sigma(E_{\rm calib}/E_{\rm true})$ for $E_{\rm ecal} \neq 0$",fontsize=20)
    plt.axis([0,160,0,0.6])
    plt.legend(loc="upper right")
    major_ticks = np.arange(0, 200, 20)
    minor_ticks = np.arange(0, 200, 2)
